(function (root, factory) {
    ...
    exports.isOutput = jphps_isOutput;
    ...
    function jphps_build(template) {
        ...
        var lines = String(template).split(/\n\r?/).map(function (line, index, array) {
            ...
            if (jphps_isOutput(line)) {
                return line.replace(/(!?#)\{("([^\\"]|(\\.))*"|'([^\\']|(\\.))*'|[^}]*)\}/g, function (all, flag, value) {
                    switch (flag) {
                        case '#':// 需要转义，防止 XSS
